...
from modules.nn.selfAttention import FeedForward, MultiHeadAttention, Norm, DecoderLayer
...
class FeatureExtractor(nn.Module):
    def __init__(self, embeddings=None, num_embeddings=0, **kwargs):
        ...
        self.encoder2 = RNNEncoder(input_size=encoder_input_size,
                                  rnn_size=encoder_size,
                                  num_layers=encoder_layers,
                                  bidirectional=bidirectional,
                                  dropout=encoder_dropout)
        ...
        self.norm_1_2 = Norm(encoder_input_size)
        self.norm_2_2 = Norm(encoder_input_size)
        self.norm_3_2 = Norm(encoder_size*2)
    ...
    def forward(self, x, lengths):
        ...
        for i in range(self.recurrent_steps):
            embeddings = self.embedding(x.type('torch.cuda.LongTensor'))
            embeddings = self.norm_1_2(embeddings)
            
            new_input = self.attentionDecoder(embeddings, encoder_outputs, lengths)
            new_input = self.norm_2_2(new_input)

            encoder_outputs, last_output = self.encoder2(new_input, lengths)
            encoder_outputs = self.norm_3_2(encoder_outputs)
